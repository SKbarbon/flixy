...
class Switch (object):
	...
	def __init__(self, width=50, height=35, bgcolor="white", value=False, on_active = None, on_disactive = None):
		self.__SelfView = ui.Switch()
		...
		self.on_disactive = on_disactive
		self.parent = None
		self.update()
	...
	def update(self):
		...
		def ACTION(self):
			if s.value:
				ss.value = True
				if ss.on_active != None: threading.Thread(target=ss.on_active).start()
			else:
				ss.value = False
				if ss.on_disactive != None: threading.Thread(target=ss.on_disactive).start()
